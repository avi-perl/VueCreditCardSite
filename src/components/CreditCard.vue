<template>
  <div class="card cardFront" :class="{'cardRedacted': card.redacted}">
    <template v-if="card.redacted">
      <div class="cardNumber">**** **** **** ****</div>
    </template>
    <template v-else>
      <div class="cardNumber">{{ card.number.replace(/-/g, " ") }}</div>
      <div class="cardExpiryDate">
        {{ card.expiry.month }}/{{ card.expiry.year }}
      </div>
      <div class="cardOwner">{{ card.name }}</div>
      <img src="images/visa-logo.png" class="cardLogo" />
    </template>
  </div>
</template>
